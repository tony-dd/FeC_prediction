...
dataset = []
for i in range(9):
    data = MD17('./data/energy_force_data/Fe' + str(i+1) + 'Cx_prop.db')
    dataset.append(data)

L_e, L_f = [], []

for i in range(len(dataset)):
    forcetut = './cuda1_forcetut_' + str(i+1)
    ...
    for j in range(len(dataset)):
        if i == j:
            continue
        sets.append(dataset[j])
    train_vali_data = ConcatAtomsData(datasets=sets)
